...
import { AuthenticateOptions, AuthenticationToken } from '../authenticate.d';
import { apiPath, get, post } from '../core';
import { encodeAuthentication } from '../authentication';


export const getTokens: (args: AuthenticateOptions) => Promise<AuthenticationToken[]> = async ({ username, password, config }) => {
  let tokens;
