...
The `withFile` HOC adds the ability to create a file, with the filepath and contents.
  ...
  return (
    <>
      <Paper style={{marginBottom: '1em', padding: '1.3em'}}>
        <FormInput
          id='filepath' label='filepath' type='text'
          onBlur={(e) => setFilepath(e.target.value)}
        />
        <FormInput
          id='defaultContent' label='defaultContent' type='text'
          onBlur={(e) => setDefaultContent(e.target.value)}
        />
